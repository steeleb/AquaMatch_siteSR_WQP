...
#' non-CONUS HUC4's, assign a NHD identifier to each site to associate them with 
...
#' fields from NHDPlusV2 or NHD Best Resolution file
#' 
#' 
add_NHD_waterbody_to_sites <- function(sites_with_huc, huc4) {
  ...
  if (huc4 < 1900) {
    ...
    huc4_wbd <- st_read(file.path("4_compile_sites/nhd/",
      ...
      select(nhd_id = permanent_identifier, gnis_id, gnis_name, fcode, areasqkm) %>% 
      rowid_to_column("wbd_id") %>% 
  ...
  if (nrow(invalid) > 0) {
    ...
    wbd_less <- huc4_wbd[!huc4_wbd$comid %in% invalid$comid, ]
  ...
  if (nrow(huc4_lake_points) > 0) {
    ...
    unmatched <- unmatched %>%
      ...
      mutate(across(all_of(c("nhd_id", "gnis_id", "gnis_name", "areasqkm")),
    ...
    assignment <- rbind(matched, unmatched) %>%
      ...
      # distance info)
      mutate(flag_wbd = case_when(!is.na(nhd_id) & is.na(dist_to_wbd) ~ 0,
                                  !is.na(nhd_id) & !is.na(dist_to_wbd) ~ 1,
                                  is.na(nhd_id) & !is.na(dist_to_wbd) ~ 2))
