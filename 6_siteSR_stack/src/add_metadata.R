...
add_metadata <- function(yaml,
  ...
  # get file using ext
  file <- collated_files[grepl("point", collated_files)]
  # load file
  ...
  # if there are DSWE3 files, print that they will not be collated
  if (nrow(df %>% filter(DSWE == 'DSWE3')) > 0) {
    message("This workflow is not set up to collate/process DSWE3 files.")
  }
